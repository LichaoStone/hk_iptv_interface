...
public class LocalHttpClient {
	...
	protected static CloseableHttpClient httpClient = HttpClientFactory.createHttpClient(400,200,timeout,retryExecutionCount);
	 ...
	 * @param jzf
	 */
	public static void initMchKeyStore(String mch_id,byte[] certificate){
		try {
			KeyStore keyStore = KeyStore.getInstance("PKCS12");
			InputStream instream = IOUtils.byteToInputStream(certificate);
			 keyStore.load(instream,mch_id.toCharArray());
			 instream.close();
			 CloseableHttpClient httpClient = HttpClientFactory.createKeyMaterialHttpClient(keyStore, mch_id,timeout,retryExecutionCount);
			 httpClient_mchKeyStore.put(mch_id, httpClient);
	 ...
	 * @return result
	 */
	public static <T> T keyStoreExecuteXmlResult(String mch_id,byte[] certificate,HttpUriRequest request,Class<T> clazz){
		String uriId = loggerRequest(request);
		...
		try {
			if (httpClient_mchKeyStore.get(mch_id) == null) {
				//如果内存里无数据，则初始化
				initMchKeyStore(mch_id, certificate);
			}
